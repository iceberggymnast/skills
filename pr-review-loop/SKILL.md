...
- PR이 건드리는 시스템 이름으로 레포의 `.md`를 검색한다
  (세이브를 건드리면 `save`·`세이브`, 항로면 `route`·`항로` 식으로)
...
5. **대조 결과를 적재한다.** 적재처는 `~/.claude/comprehension-debt.md` —
   ...
   ```
   | 2026-08-12 | PR #47 | MyGame/UE5-C++ | 상태 관리 | 일치 |
   | 2026-08-12 | PR #47 | MyGame/UE5-C++ | 세이브 스키마 | 짚었으나 틀림 |
   | 2026-08-12 | PR #47 | MyGame/UE5-C++ | 액터 생명주기 | 모름 |
   ```
   ...
   | AI만 짚음 (사용자가 "모름") | `모름` |
   | **사용자만 짚음** | 집계에 남기지 않는다 — 이건 사용자 부채가 아니라 **리뷰가 놓친 것**이다. (b)로 간다 |

   (b) **부채가 되는 것만 정식 항목으로** — `모름` · `짚었으나 틀림`, 그리고
   **사용자만 짚음**(리뷰 측 누락). 형식은 `comprehension-debt.md`의
...
[ ] 스폰 시드가 0이 아님
[ ] SettlementWidgetClass 할당됨
[ ] 재출항 시 스크랩이 0으로 리셋
[ ] 정산 화면이 1회만 뜨고 게임이 멈췄다 재개됨
```
...
| `?`의 종류 | 처리 |
|---|---|
| **확인할 수단이 없다** — "항로를 켜고 끄는 버튼 자체가 없어서 확인 못 함" | 검증 실패가 아니라 **하네스 부재**다. 따로 모아 하네스 백로그로 넘긴다. 같은 항목이 반복되면 그 부분의 디버그 도구를 먼저 만든다 |
