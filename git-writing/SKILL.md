...
> `TreePanelWidget`은 자체 `bPurchased` 플래그를 버리고 표시 전용으로 전환, 구매 여부는 매번 RunState에 질의. 위젯 로컬 상태는 레벨 전환·세이브에서 사라지고, Settlement(투자 점수)·Event(`HasTreeNode` 조건)가 같은 상태를 읽어야 해서 공유 계약③으로 노출했다.

**후 (문장마다 정보 하나, 서술어까지 완결):**
> `TreePanelWidget`은 이제 표시만 담당합니다. 구매 여부는 매번 RunState에 물어봅니다. 위젯이 직접 들고 있던 `bPurchased` 플래그는 레벨 전환이나 세이브에서 사라지는 값이라 버렸습니다. Settlement와 Event도 같은 구매 상태를 읽어야 해서, 조회 함수들을 공유 계약③(위젯–상태 간 공유 API 규약)으로 노출했습니다.
...
`RATIONALE.md`는 원작성자의 관측 기록이라 배포에 포함되지 않는다. 이 스킬을 새로 받았다면 자기 관측 기록으로 직접 만든다.
