...
**`RATIONALE.md`를 가진 스킬만 잰다.** 그 파일이 있다는 것은 성공 기준을 선언했다는 뜻이고, 선언이 없으면 발동률이 낮은 것이 문제인지 정상인지 판정할 근거가 없다.

`RATIONALE.md`가 없는 스킬은 목록에 올리되 **"측정 불가 — 기준 없음"**으로 적는다. 임의 기준을 지어내 평가하지 않는다.
...
- **훅 주입** — 각 훅이 내보내는 마커 문구(`[예측 대조]`, `[CS 복습]` 등). 훅은 스킬 파일을 로드하지 않고도 절차를 시작시키므로, 로드만 세면 훅으로 도는 스킬을 크게 과소평가한다
...
**"안 잡혔다"와 "안 돌았다"는 출력이 같다.** 낮은 수치를 보고 트리거를 고치기 전에 이 목록부터 확인한다. 계기를 못 믿으면 그 항목은 "측정 불가"로 두고 넘어간다.
...
각 스킬의 `RATIONALE.md`가 적재처를 가리킨다. 그 파일에서 같은 기간의 항목 수를 센다.
...
- `RATIONALE.md`가 없는 스킬에 임의 기준을 지어내 평가하는 것
